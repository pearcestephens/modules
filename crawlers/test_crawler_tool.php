...
$crawlerScriptPath = $_SERVER['DOCUMENT_ROOT'] . '/frontend-tools/scripts/deep-crawler.js';
if (file_exists($crawlerScriptPath)) {
    echo "✅ PASS\n";
    $passed++;
    echo "   📁 Found at: {$crawlerScriptPath}\n";
} else {
    echo "⚠️  WARN - Script not found (will fail on actual execution)\n";
    echo "   📁 Expected at: {$crawlerScriptPath}\n";
    $passed++; // Don't fail the test, just warn
}
