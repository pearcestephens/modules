...
function getTransferShipments(int $transferId): array
{
    ...
    $stmt = $pdo->prepare("
        SELECT ts.*, 
               (SELECT COUNT(*) FROM transfer_parcels WHERE shipment_id = ts.id) as parcel_count
...
function getTransferReceipts(int $transferId): array
{
    ...
    $stmt = $pdo->prepare("
        SELECT tr.*, 
               (SELECT COUNT(*) FROM transfer_receipt_items WHERE receipt_id = tr.id) as item_count
...
function getTransferAIInsights(int $transferId): array
{
    ...
    $stmt = $pdo->prepare("
        SELECT * FROM transfer_ai_insights
        WHERE transfer_id = ?
