...
final class ConsignmentService
{
    ...
    public function recent(int $limit = 50): array {
        ...
        $stmt = $this->ro->prepare(
            "SELECT id, ref_code, status, origin_outlet_id, dest_outlet_id, created_at, updated_at
             FROM consignments
             ORDER BY id DESC
             LIMIT :lim"
        );
    ...
    public function get(int $id): ?array {
        $stmt = $this->ro->prepare(
            "SELECT c.*,
                    (SELECT COUNT(*) FROM consignment_items i WHERE i.consignment_id = c.id) AS item_count
             FROM consignments c
             WHERE c.id = :id
             LIMIT 1"
        );
