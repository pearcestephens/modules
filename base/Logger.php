...
class CISLogger {
    ...
    private static ?SecretRedactor $redactor = null;
    ...
    public static function init(): void {
        if (self::$sessionId === null) {
            ...
            // Initialize redactor with known sensitive env keys
            self::$redactor = new SecretRedactor([
                'DB_PASSWORD','VEND_ACCESS_TOKEN','VEND_REFRESH_TOKEN','VEND_CLIENT_SECRET','SENDGRID_API_KEY',
                'OPENAI_API_KEY','GOOGLE_MAPS_API_KEY','DEPUTY_ACCESS_TOKEN','ENCRYPTION_KEY','SESSION_SECRET','JWT_SECRET'
            ]);
        }
    }
    ...
    public static function action(
        ...
        try {
            ...
            $msg = "[CISLogger::action] Failed: " . $e->getMessage();
            if (self::$redactor) { $msg = self::$redactor->redact($msg); }
            error_log($msg);
            return null;
        }
    }
    ...
    public static function ai(
        ...
        try {
            ...
            $msg = "[CISLogger::ai] Failed: " . $e->getMessage();
            if (self::$redactor) { $msg = self::$redactor->redact($msg); }
            error_log($msg);
            return null;
        }
    ...
    public static function security(
        ...
        try {
            return Database::insert('cis_security_events', [
            ...
            $msg = "[CISLogger::security] Failed: " . $e->getMessage();
            if (self::$redactor) { $msg = self::$redactor->redact($msg); }
            error_log($msg);
            return null;
        }
    ...
    public static function performance(
        ...
        try {
            ...
            $msg = "[CISLogger::performance] Failed: " . $e->getMessage();
            if (self::$redactor) { $msg = self::$redactor->redact($msg); }
            error_log($msg);
            return null;
        }
    ...
    /**
     * Log low-level user behavior event (from server-side or backend detections)
     * Example events: click, keypress_meta (never raw text), scroll, visibility, suspicious
     */
    public static function behavior(
        string $eventType,
        array $details = [],
        ?string $pageUrl = null,
        ?int $occurredAtMs = null
    ): ?int {
        self::init();
        try {
            return Database::insert('cis_user_events', [
                'user_id' => $_SESSION['user_id'] ?? null,
                'session_id' => self::$sessionId,
                'event_type' => $eventType,
                'event_data' => json_encode($details, JSON_UNESCAPED_UNICODE),
                'page_url' => $pageUrl ?? ($_SERVER['REQUEST_URI'] ?? null),
                'occurred_at_ms' => $occurredAtMs ?? (int) round(microtime(true) * 1000),
                'ip_address' => $_SERVER['REMOTE_ADDR'] ?? null,
                'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? null,
                'trace_id' => self::$traceId
            ]);
        } catch (Exception $e) {
            $msg = '[CISLogger::behavior] Failed: ' . $e->getMessage();
            if (self::$redactor) { $msg = self::$redactor->redact($msg); }
            error_log($msg);
            return null;
        }
    ...
    public static function botPipeline(
        ...
        try {
            ...
            $msg = "[CISLogger::botPipeline] Failed: " . $e->getMessage();
            if (self::$redactor) { $msg = self::$redactor->redact($msg); }
            error_log($msg);
            return null;
        }
    ...
    private static function startSession(): void {
        ...
        try {
            ...
            $msg = "[CISLogger::startSession] Failed: " . $e->getMessage();
            if (self::$redactor) { $msg = self::$redactor->redact($msg); }
            error_log($msg);
        }
    }
    ...
    public static function endSession(string $reason = 'logout'): void {
        try {
            ...
            $msg = "[CISLogger::endSession] Failed: " . $e->getMessage();
            if (self::$redactor) { $msg = self::$redactor->redact($msg); }
            error_log($msg);
        }
    }
